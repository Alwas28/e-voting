...
<div class="p-6 max-w-3xl mx-auto space-y-6">
  ...
  <div class="flex items-center gap-4">
    ...
    <div>
      ...
      <p class="text-slate-500 text-sm mt-0.5">No. Urut {{ $candidate->number }} — {{ $candidate->name }}</p>
    </div>
  </div>
  ...
  <form action="{{ route('admin.candidates.update', $candidate) }}" method="POST" enctype="multipart/form-data"
    ...
    <div class="relative z-20 bg-white rounded-2xl border border-slate-200 shadow-sm">
      <div class="px-6 py-4 border-b border-slate-100">
        ...
        <p class="text-xs text-slate-400 mt-0.5">Cari untuk mengganti alumni kandidat</p>
      </div>
      <div class="px-6 py-5">
        @include('admin.candidates.partials.alumni-search', [
            'selectedAlumni'  => $candidate->alumni,
            'exceptAlumniId'  => $candidate->alumni_id,
        ])
      </div>
    </div>
      ...
      <div class="px-6 py-5 space-y-5">
        ...
        <div class="flex items-start gap-6">
          ...
          <div class="shrink-0 flex flex-col items-center gap-2">
            ...
            <p class="text-xs text-slate-400 text-center">Klik untuk ganti foto<br>(kosongkan jika tidak berubah)</p>
          </div>

          {{-- Nomor urut --}}
          <div class="flex-1">
            <label class="block text-xs font-medium text-slate-600 mb-1.5">Nomor Urut <span class="text-red-500">*</span></label>
            <input type="number" name="number" value="{{ old('number', $candidate->number) }}" min="1" max="99"
        ...
        <label class="flex items-center gap-2.5 cursor-pointer select-none">
          ...
          <span class="text-sm text-slate-600">Kandidat aktif</span>
        </label>
...
// Isi dengan profil yang sudah ada
